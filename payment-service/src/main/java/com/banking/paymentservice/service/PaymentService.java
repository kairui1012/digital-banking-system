...
public class PaymentService {
    ...
    public PaymentOrderResponse createPaymentOrder(@Valid CreatePaymentRequest request) throws RazorpayException {
        ...
        // RazorpayClient 里的参数是 razorpay 的 key id 还有 key secret
        RazorpayClient razorpayClient = new RazorpayClient(keyId,keySecret);

        // Converted Amount
        int convertedAmount = request.getAmount().multiply(BigDecimal.valueOf(100)).intValue();

        JSONObject orderRequests = new JSONObject();
        orderRequests.put("amount",convertedAmount);
        orderRequests.put("currency","USD/SGD");
        orderRequests.put("receipt","rcpt_" + System.currentTimeMillis() + UUID.randomUUID().toString().replace("-","").substring(0,10));
        Order razerpayOrder = razorpayClient.orders.create(orderRequests);
        log.info("Razorpay order created: {}",razerpayOrder.get("id").toString());

        // Save payment record
        Payment payment = new Payment();
        payment.setRazorpayOrderId(razerpayOrder.get("id").toString());
    ...
    public void handleWebhook(Map<String, Object> payload) {
        ...
        if ("payment.cptured".equals(event)){
